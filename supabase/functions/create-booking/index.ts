...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isAdminBooking = body.created_by_admin === true;
    let adminVerified = false;

    if (isAdminBooking) {
      const authHeader = req.headers.get("Authorization");
      adminVerified = await verifyAdmin(supabase, authHeader);
      if (!adminVerified) {
        return new Response(
    ...
    // Non-admin bookings: honeypot + turnstile + rate limiting + member verification
    if (!adminVerified) {
      // Honeypot check
    ...
    if (!adminVerified) {
      ...
      if (count !== null && count >= 3) {
        return new Response(
          JSON.stringify({ error: "Zu viele Anfragen. Bitte warten Sie kurz." }),
    ...
    // --- Server-side core time limit enforcement (skip for admin bookings) ---
    if (!adminVerified) {
    ...
    const { data, error } = await supabase.from("bookings").insert({
      ...
      is_joined: false,
      created_by_admin: adminVerified,
      created_by_ip: adminVerified ? null : clientIp,
    }).select("id").single();
