...
-- Server-side booking window validation trigger
...
BEGIN
  ...
  -- Calculate slot time (date + start_hour in UTC, matching frontend behavior)
  _slot_time := (NEW.date || 'T' || lpad(NEW.start_hour::text, 2, '0') || ':00:00')::timestamp;
  _now := now();
...
-- Attach trigger
CREATE TRIGGER trg_validate_booking_window
  BEFORE INSERT ON public.bookings
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_booking_window();
